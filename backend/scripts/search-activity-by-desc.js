...
(async ()=>{
  try{
    const logs = await prisma.activityLog.findMany({ where: { description: { contains: 'Marked occurrence' } }, orderBy: { createdAt: 'desc' }, take: 20 });
    console.log('Found', logs.length, 'entries matching description:');
    for(const l of logs){
      console.log('---');
      console.log('id:', l.id);
