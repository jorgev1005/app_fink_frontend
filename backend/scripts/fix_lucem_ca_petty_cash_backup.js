...
async function main() {
  ...
  const user = await prisma.user.findFirst({ where: { email: 'admin@admin.com' }, select: { id: true } });
  if (!user) {
    throw new Error('No se encontró el usuario admin@admin.com para registrar el ajuste.');
  }
