...
export async function generatePurchaseOrderPDFBuffer(options: PurchaseOrderPDFOptions): Promise<{ buffer: Buffer; orderNumber: string }> {
    ...
    return new Promise((resolve, reject) => {
        ...
        let cleanNotes = (notes && typeof notes === 'string') ? notes : '';
        ...
        if (!cleanNotes) {
            cleanNotes = 'Favor confirmar recepción de la orden y fecha estimada de despacho.';
        }
        ...
        doc.fontSize(6.5).fillColor(GRAY).font('Helvetica')
           .text(cleanNotes, LEFT + 8, totY + 18, { width: noteW - 15, lineBreak: false })
