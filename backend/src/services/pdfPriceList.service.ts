...
export async function generatePriceListPDFBuffer(options: PriceListPDFOptions): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
        ...
        // ── CÓDIGO DE CONTROL ADMINISTRATIVO (LP-AAMMDD-TIPO) ───────
        // F15: Flete/Aumento +15% | D05: Descuento -5% | B00: Base Fábrica 0%
        ...
        if (adjustmentPercentage > 0) {
            codeTag = `F${String(Math.round(adjustmentPercentage)).padStart(2, '0')}`;
        } else if (adjustmentPercentage < 0) {
            codeTag = `D${String(Math.round(Math.abs(adjustmentPercentage))).padStart(2, '0')}`;
        }
        const controlCode = `LP-${datePart}-${codeTag}`;
        ...
        doc.fontSize(11).fillColor('white').font('Helvetica-Bold')
           .text('LISTA DE PRECIOS OFICIAL', 310, 56, { width: 220, align: 'right', lineBreak: false });
        
        if (projectName) {
            doc.fontSize(8).fillColor(GREEN).font('Helvetica-Bold')
               .text(`PROYECTO: ${projectName.toUpperCase()}`, 310, 70, { width: 220, align: 'right', lineBreak: false });
        }

        doc.fontSize(6.5).fillColor('#9ca3af').font('Helvetica')
           .text(`Fecha de emisión: ${fmtDate(ahora)}`, 310, 83, { width: 220, align: 'right', lineBreak: false });

        doc.fontSize(7).fillColor('#38bdf8').font('Helvetica-Bold')
           .text(`Ref / Control: ${controlCode}`, 310, 95, { width: 220, align: 'right', lineBreak: false });
        ...
        for (let i = range.start; i < range.start + range.count; i++) {
            ...
            doc.fontSize(6.5).fillColor(GRAY).font('Helvetica')
               .text(`Grupo Aludra © ${ahora.getFullYear()} | Inversiones Lucem C.A. | Control: ${controlCode}`, LEFT, yFooter + 4, { width: W - 90, align: 'left', lineBreak: false });
            doc.text(`Pág. ${i + 1} de ${range.count}`, LEFT + W - 80, yFooter + 4, { width: 80, align: 'right', lineBreak: false });
            doc.restore();
        }
